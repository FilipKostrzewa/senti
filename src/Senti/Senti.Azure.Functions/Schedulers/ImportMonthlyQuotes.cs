...
{
    public class ImportMonthlyQuotes
    {
        private readonly ILogger _logger;
        private readonly Core.ImportQuotes _importQuotes;

        public ImportMonthlyQuotes(ILoggerFactory loggerFactory, Core.ImportQuotes importQuotes)
        {
            _logger = loggerFactory.CreateLogger<ImportQuotes>();
            _importQuotes = importQuotes;
        }

        [Function(nameof(ImportMonthlyQuotes))]
        public async Task Run([TimerTrigger("%Cron_ImportMonthlyQuotes%")] TimerInfo myTimer)
        {
